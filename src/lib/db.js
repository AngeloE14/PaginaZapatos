...
import path from 'path'; // File touched to trigger reload

// Conectar a la base de datos (o crearla si no existe)
const db = new Database(path.join(process.cwd(), 'database.sqlite'));

// Configurar pragmas para mejor rendimiento y seguridad
...
if (productCount.count === 0) {
  const insertProduct = db.prepare('INSERT INTO products (name, price, image, description, sizes) VALUES (?, ?, ?, ?, ?)');
  ...
  for (const prod of productsToSeed) {
    insertProduct.run(prod.name, prod.price, prod.image, prod.desc, prod.sizes);
  }

  const insertService = db.prepare('INSERT INTO services (name, description, price) VALUES (?, ?, ?)');
  insertService.run('Cambio de suela', 'Reemplazo completo de la suela desgastada', 25.00);
  insertService.run('Reparación de costuras', 'Arreglo de costuras rotas o desgastadas', 15.00);
  insertService.run('Reemplazo de cordones', 'Cambio de cordones por unos nuevos', 8.00);
  insertService.run('Limpieza profunda', 'Limpieza completa y restauración de color', 12.00);
  insertService.run('Reparación de cremallera', 'Arreglo o reemplazo de cremallera dañada', 18.00);
  insertService.run('Refuerzo de talón', 'Refuerzo interno para mayor durabilidad', 10.00);
}
